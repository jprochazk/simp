...
fn parse_stmt(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Stmt> {
    match c.kind() {
...
fn parse_stmt_fn(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Stmt> {
    assert!(c.eat(KW_FN));
...
fn parse_stmt_let(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Stmt> {
    assert!(c.eat(KW_LET));
...
fn parse_stmt_expr(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Stmt> {
...
fn parse_expr(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
    match c.kind() {
...
fn parse_expr_if(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
    assert!(c.eat(KW_IF));
...
fn parse_expr_or(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
...
fn parse_expr_and(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
...
fn parse_expr_eq(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
...
fn parse_expr_ord(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
...
fn parse_expr_add(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
...
fn parse_expr_mul(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
...
fn parse_expr_unary(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
    let op = match c.kind() {
...
fn parse_expr_postfix(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
...
fn parse_expr_call(c: &mut Cursor, ast: &mut AstBuilder, callee: Expr) -> Result<Expr> {
    let args = parse_arg_list(c, ast)?;

    Ok(Expr::Call(Box::new(ExprCall { callee, args })))
}

fn parse_arg_list(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Vec<Expr>> {
    let args = parse_paren_list(c, ast, parse_expr)?;
    Ok(args)
}

fn parse_expr_primary(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
    match c.kind() {
...
fn parse_expr_int(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
    let _ = ast;
...
fn parse_expr_str(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
    ...
    Ok(Expr::Str(Box::new(ExprStr { value: id })))
}

fn parse_expr_ident(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
...
fn parse_expr_block(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
    let inner = parse_block(c, ast)?;
    Ok(Expr::Block(Box::new(ExprBlock { inner })))
}

fn parse_expr_group(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Expr> {
...
fn parse_block(c: &mut Cursor, ast: &mut AstBuilder) -> Result<Block> {
    c.must(TOK_LBRACE)?;
    let mut body = Vec::new();
