...
pub fn parse<'a>(code: &str, bump: &'a Bump) -> Result<Ast<'a>> {
    let mut ast = Ast::builder(bump);
...
fn parse_program<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Program<'a>> {
    let mut body = vec![in ast.bump()];
    while !c.at(TOK_EOF) {
...
fn parse_stmt<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Stmt<'a>> {
    match c.kind() {
...
fn parse_stmt_fn<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Stmt<'a>> {
    assert!(c.eat(KW_FN));

    let name = parse_ident(c, ast)?;
    let params = parse_param_list(c, ast)?;
    let body = parse_block(c, ast)?;

    Ok(Stmt::Fn(ast.alloc(StmtFn { name, params, body })))
}

fn parse_param_list<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Vec<'a, IdentId>> {
...
where
    F: FnMut(&mut Cursor, &mut AstBuilder<'a>) -> Result<T>,
{
    let mut list = vec![in ast.bump()];
...
fn parse_stmt_let<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Stmt<'a>> {
    assert!(c.eat(KW_LET));
    ...
    Ok(Stmt::Let(ast.alloc(StmtLet { name, value })))
}

fn parse_stmt_expr<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Stmt<'a>> {
    ...
    Ok(Stmt::Expr(ast.alloc(expr)))
}

fn parse_expr<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
    match c.kind() {
...
fn parse_expr_if<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
    assert!(c.eat(KW_IF));

    let mut branches = vec![in ast.bump(); parse_if_branch(c, ast)?];
    let mut tail = None;
    while c.eat(KW_ELSE) {
        if c.eat(KW_IF) {
            branches.push(parse_if_branch(c, ast)?);
        } else {
            tail = Some(parse_block(c, ast)?);
            break;
        }
    }

    Ok(Expr::If(ast.alloc(ExprIf { branches, tail })))
}

fn parse_if_branch<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<IfBranch<'a>> {
...
fn parse_expr_or<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
    let mut lhs = parse_expr_and(c, ast)?;
    while c.eat(OP_OR) {
        let op = BinaryOp::Or;
        let rhs = parse_expr_and(c, ast)?;
        lhs = Expr::Binary(ast.alloc(ExprBinary { lhs, op, rhs }))
    }

    Ok(lhs)
}

fn parse_expr_and<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
...
fn parse_expr_eq<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
...
fn parse_expr_ord<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
...
fn parse_expr_add<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
...
fn parse_expr_mul<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
...
fn parse_expr_unary<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
    let op = match c.kind() {
    ...
    c.advance(); // eat `op`
    let rhs = parse_expr_unary(c, ast)?;
    Ok(Expr::Unary(ast.alloc(ExprUnary { op, rhs })))
}

fn parse_expr_postfix<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
...
fn parse_expr_call<'a>(
    ...
    Ok(Expr::Call(ast.alloc(ExprCall { callee, args })))
}

fn parse_arg_list<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Vec<'a, Expr<'a>>> {
    let args = parse_paren_list(c, ast, parse_expr)?;
    Ok(args)
}

fn parse_expr_primary<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
    match c.kind() {
...
fn parse_expr_int<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
    let _ = ast;
    ...
    Ok(Expr::Int(ast.alloc(ExprInt { value })))
}

fn parse_expr_str<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
    ...
    Ok(Expr::Str(ast.alloc(ExprStr { value: id })))
}

fn parse_expr_ident<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
    let token = c.must(LIT_IDENT)?;
    let name = c.lexeme(token);
    let id = ast.intern_ident(name);
    Ok(Expr::Ident(ast.alloc(ExprIdent { name: id })))
}

fn parse_expr_block<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
    let inner = parse_block(c, ast)?;
    Ok(Expr::Block(ast.alloc(ExprBlock { inner })))
}

fn parse_expr_group<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Expr<'a>> {
...
fn parse_block<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<Block<'a>> {
    c.must(TOK_LBRACE)?;
    let mut body = vec![in ast.bump()];
...
fn parse_ident<'a>(c: &mut Cursor, ast: &mut AstBuilder<'a>) -> Result<IdentId> {
    let token = c.must(LIT_IDENT)?;
    let lexeme = c.lexeme(token);
    let id = ast.intern_ident(lexeme);
    Ok(id)
}
